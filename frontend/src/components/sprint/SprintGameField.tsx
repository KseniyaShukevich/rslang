...
const SprintGameField: React.FC<ISprintProps> = (props) => {
  ...
  return (
    <>
      {isPrepareLayout && (
        <PrepareLayout
          setIsPrepareLayout={setIsPrepareLayout}
          isAudio={isAudio}
        />
      )}
      {!isPrepareLayout && (
        <Box className={classes.root}>
          <Box className={classes.horizontalRow}>
            <Box style={{ flex: "0 1 33%", textAlign: "left" }}>
              <Timer maxValue={60} action={handleTimerEnd} isAudio={isAudio} />
            </Box>
            <Typography
              variant="h6"
              style={{ flex: "0 1 33%", textAlign: "center" }}
            >
              {score}
            </Typography>
            <Box style={{ flex: "0 1 33%", textAlign: "right" }}>
              <Typography variant="h6">{`Left words: ${restWords}`}</Typography>
            </Box>
          </Box>
            ...
            <Box className={classes.horizontalRow}>
              <div style={{ flex: "0 1 60%", textAlign: "right" }}>
                <CorrectAnswerIcons
                  ...
                  isSingleSequence={isSingleSequence}
                />
              </div>
